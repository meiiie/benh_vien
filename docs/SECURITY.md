...
- Khi client gửi `Accept: application/fhir+json`, lỗi `401 UNAUTHENTICATED`, `403 FORBIDDEN` và `403 PATIENT_ACCESS_DENIED` ở tầng auth/RBAC được trả theo FHIR `OperationOutcome`; cùng lúc hệ thống vẫn ghi `access.denied` vào audit trail cho các lần 403 có actor hợp lệ.
...
- API chỉ mặc định `x-purpose-of-use` là `TREATMENT` khi header bị thiếu hoặc để trống; nếu client gửi giá trị khác `TREATMENT`, `AUDIT` hoặc `OPERATIONS`, request bị chặn bằng `400 INVALID_PURPOSE_OF_USE` hoặc FHIR `OperationOutcome` để tránh ghi audit sai mục đích truy cập.
