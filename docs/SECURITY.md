...
- `ClinicalDocument`, `Observation` và `RecordTransfer` dùng cùng rào ABAC theo bệnh nhân cho cả list/create/read/export/update, nên người dùng điều trị không thể đi vòng qua endpoint con để xem tài liệu, chỉ số hoặc gói chuyển hồ sơ của bệnh nhân ngoài tổ chức.
