...
- Các endpoint lâm sàng như bệnh nhân, Provider Directory, lượt khám, dị ứng/cảnh báo `AllergyIntolerance`, chẩn đoán `Condition`, chỉ định dịch vụ `ServiceRequest`, công việc thực thi `Task`, thủ thuật/hoạt động `Procedure`, chỉ số `Observation`, báo cáo kết quả `DiagnosticReport`, nghiên cứu hình ảnh `ImagingStudy`, chỉ định thuốc `MedicationRequest`, dùng thuốc thực tế `MedicationAdministration`, tài liệu và Bundle đều đi qua kiểm tra quyền ở API.
...
- `MedicationAdministration` có quyền đọc/tạo/xuất riêng để tách “đã dùng thuốc” khỏi “đã kê thuốc”. Điều dưỡng có thể ghi nhận/đọc trong phạm vi điều trị nhưng không xuất FHIR; bác sĩ/quản trị có thể xuất khi mục đích sử dụng là điều trị.
