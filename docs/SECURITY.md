...
- Prod-like Docker compose không publish trực tiếp cổng API ra host; web edge là điểm vào công khai, còn API chỉ nằm trên mạng nội bộ compose để giảm bề mặt tấn công.
...
- Release workflow chỉ publish image GHCR theo semantic version, không publish tag `latest`, để môi trường triển khai không vô tình nhận artifact mutable ngoài kế hoạch kiểm thử.
- Image release bật provenance attestation mức `mode=max` và SBOM attestation cho cả API lẫn web, giúp truy vết nguồn build và thành phần phần mềm khi kiểm tra chuỗi cung ứng.
...
- Web runtime Nginx tắt `server_tokens` và chuyển tiếp `X-Request-Id` khi proxy API/docs; API vẫn là lớp kiểm tra/sinh request id an toàn trước khi ghi log/audit metadata.
