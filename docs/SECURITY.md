...
- `RecordTransfer` kiểm consent và endpoint FHIR Bundle của đơn vị nhận trước khi tạo/gửi, ghi audit khi liệt kê/tạo/xem/gửi/lỗi/thử lại/xác nhận nhận/xuất FHIR `Task`; retry worker ghi audit với `purposeOfUse = OPERATIONS` khi tự đưa gói lỗi đã đến hạn về hàng đợi, và bảng vận hành không lưu bản sao đầy đủ của Bundle.
