...
- `RecordTransfer` kiểm consent và endpoint FHIR Bundle của đơn vị nhận trước khi tạo/gửi, ghi audit khi liệt kê/tạo/xem/gửi/lỗi/thử lại/xác nhận nhận/xuất FHIR `Task`; mỗi lần gửi tạo một delivery attempt có endpoint đích, FHIR Bundle đích, số lần thử và idempotency key để chống gửi trùng khi worker liên thông thật được bổ sung. Retry worker ghi audit với `purposeOfUse = OPERATIONS` khi tự đưa gói lỗi đã đến hạn về hàng đợi, và các bảng vận hành không lưu bản sao đầy đủ của Bundle.
