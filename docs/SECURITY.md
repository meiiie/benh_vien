...
- `Encounter`, `AllergyIntolerance`, `Condition`, `ClinicalDocument`, `Observation`, `ServiceRequest`, `Task` và `RecordTransfer` dùng cùng rào ABAC theo bệnh nhân cho list/create/read/export/update tương ứng, nên người dùng điều trị không thể đi vòng qua endpoint con để xem lượt khám, dị ứng, chẩn đoán, tài liệu, chỉ số, y lệnh, công việc hoặc gói chuyển hồ sơ của bệnh nhân ngoài tổ chức.
