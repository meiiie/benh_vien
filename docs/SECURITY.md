...
- Các phản hồi `403 FORBIDDEN`, `403 PATIENT_ACCESS_DENIED` và `400 INVALID_PURPOSE_OF_USE` có actor hợp lệ được ghi thành audit event `access.denied`; với lỗi theo hồ sơ bệnh nhân, sự kiện được gắn vào audit trail của bệnh nhân và khi xuất FHIR sẽ có `AuditEvent.outcome = 4` để thể hiện lần truy cập bị chặn.
...
- Ở `NODE_ENV=production`, API bắt buộc cấu hình `BVS_PUBLIC_API_BASE_URL` bằng URL HTTPS public, không dùng `localhost`, loopback, private IP hoặc link-local IP, để FHIR metadata không công bố nhầm địa chỉ nội bộ.
...
- Ở `NODE_ENV=production`, API cũng kiểm tra cấu hình chữ ký callback chuyển hồ sơ ngay khi khởi động; `BVS_RECORD_TRANSFER_CALLBACK_SECRETS_JSON` hoặc `BVS_RECORD_TRANSFER_CALLBACK_SECRET` phải tồn tại, đủ dài và không dùng placeholder/dev-only secret.
...
- `/api/v1/runtime` chỉ trả metadata tương thích web công khai; repository, môi trường, giới hạn body HTTP, trạng thái API docs và worker chuyển hồ sơ được redacted nếu request không có phiên admin/auditor với `PurposeOfUse=OPERATIONS` hoặc `PurposeOfUse=AUDIT`.
- Prod-like Docker compose không publish trực tiếp cổng API ra host; web edge là điểm vào công khai, còn API chỉ nằm trên mạng nội bộ compose để giảm bề mặt tấn công. Web runtime dùng Nginx unprivileged và listen cổng container `8080` để tránh cần quyền bind cổng đặc quyền.
...
- CI chạy `harness:secret-hygiene` để chặn file `.env` thật bị track, bảo đảm `.gitignore` vẫn chặn secret local và quét các token/private key phổ biến trong file text tracked. Đây là lớp guard tối thiểu, chưa thay thế secret scanning chuyên dụng trên GitHub hoặc nền tảng bảo mật doanh nghiệp.
- CODEOWNERS, PR template và issue templates được kiểm bằng `harness:repo-governance` để các vùng lâm sàng/vận hành nhạy cảm như domain, API, migration, CI, compose, chuẩn y tế và runbook không mất review owner, câu hỏi tuân thủ tối thiểu hoặc cảnh báo không đưa dữ liệu bệnh nhân thật/secret lên GitHub.
...
- API và web container đặt security headers nền tảng gồm `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, `Cross-Origin-Resource-Policy`, `Cross-Origin-Opener-Policy` và `Strict-Transport-Security` ở web edge.
...
- Web runtime Nginx tắt `server_tokens`, chạy trên cổng không đặc quyền `8080` và chuyển tiếp `X-Request-Id` khi proxy API/docs; API vẫn là lớp kiểm tra/sinh request id an toàn trước khi ghi log/audit metadata.
- API đặt `Cache-Control: no-store` và `Pragma: no-cache` để giảm rủi ro browser/proxy lưu nhầm response chứa dữ liệu bệnh án.
- API giới hạn request body JSON bằng `BVS_HTTP_BODY_LIMIT_BYTES`, mặc định `1048576` byte và bị chặn ngoài khoảng `1024` đến `10485760` byte để giảm rủi ro gửi payload quá lớn vào API nghiệp vụ. Tài liệu/ảnh thật nên đi qua object storage hoặc luồng upload chuyên dụng, không nhồi binary vào JSON metadata.
...
- API có error handler tập trung: client JSON thông thường vẫn nhận lỗi validation dạng `400 VALIDATION_ERROR` kèm `requestId`; client FHIR gửi `Accept: application/fhir+json` nhận FHIR `OperationOutcome.issue[]` với mã nội bộ đặt trong `issue.details.coding`, còn lỗi ngoài ý muốn trả `500 INTERNAL_SERVER_ERROR` và không trả stack trace hoặc chi tiết nội bộ.
...
- Khi client gửi `Accept: application/fhir+json`, lỗi `401 UNAUTHENTICATED`, `400 INVALID_PURPOSE_OF_USE`, `403 FORBIDDEN` và `403 PATIENT_ACCESS_DENIED` ở tầng auth/RBAC được trả theo FHIR `OperationOutcome`; cùng lúc hệ thống vẫn ghi `access.denied` vào audit trail cho các lần bị chặn có actor hợp lệ.
- Các lỗi validation payload của endpoint đăng nhập và endpoint nghiệp vụ vẫn giữ envelope `400 VALIDATION_ERROR` cho client JSON; riêng luồng FHIR dùng `OperationOutcome` để client liên thông đọc được `issue.code`, `diagnostics` và `expression` theo chuẩn FHIR R4.
...
- API chỉ mặc định `x-purpose-of-use` là `TREATMENT` khi header bị thiếu hoặc để trống; nếu client gửi giá trị khác `TREATMENT`, `AUDIT` hoặc `OPERATIONS`, request bị chặn bằng `400 INVALID_PURPOSE_OF_USE` hoặc FHIR `OperationOutcome`, đồng thời ghi `access.denied` với `purposeOfUse = OPERATIONS` để truy vết sự kiện chính sách mà không ghi nhận sai thành một lần truy cập lâm sàng hợp lệ.
...
- `RecordTransfer` kiểm consent và endpoint FHIR Bundle của đơn vị nhận trước khi tạo/gửi, ghi audit khi liệt kê/tạo/xem/gửi/lỗi/thử lại/đưa vào hàng lỗi cuối/xác nhận nhận/callback acknowledgement/xuất FHIR `Task`; mỗi lần gửi tạo một delivery attempt có endpoint đích, FHIR Bundle đích, số lần thử và idempotency key để chống gửi trùng. Khi xác nhận nhận, API lưu người xác nhận và mã biên nhận tiếp nhận trong bản ghi chuyển hồ sơ cũng như metadata audit để truy vết ai đã đóng mốc `completed`. Callback acknowledgement yêu cầu quyền hẹp `record-transfer:acknowledge`, role `integration` hoặc `admin`, mục đích sử dụng `OPERATIONS`, payload khớp cơ sở nhận và actor thuộc cơ sở nhận hoặc tài khoản admin vận hành. Nếu cấu hình `BVS_RECORD_TRANSFER_CALLBACK_SECRETS_JSON`, callback phải gửi key id qua `x-wiiicare-callback-key-id` để chọn secret riêng từng gateway; fallback một gateway là `BVS_RECORD_TRANSFER_CALLBACK_SECRET`. Callback đã ký còn phải có `x-wiiicare-callback-timestamp` và `x-wiiicare-callback-signature`; timestamp chỉ lệch tối đa 5 phút để giảm replay. Delivery worker gửi Bundle bằng HTTP `POST` khi được bật, chỉ lưu metadata phản hồi/lỗi tối thiểu và không lưu bản sao đầy đủ của Bundle trong bảng vận hành. Retry worker ghi audit với `purposeOfUse = OPERATIONS` khi tự đưa gói lỗi đã đến hạn về hàng đợi hoặc chuyển sang `dead-lettered` sau khi vượt trần retry.
- Ở `NODE_ENV=production`, endpoint FHIR nhận hồ sơ trong Provider Directory phải là URL HTTPS và không được trỏ tới `localhost`, loopback hoặc địa chỉ chỉ dùng nội bộ máy. API chặn ngay khi tạo/gửi `RecordTransfer`, còn delivery worker kiểm tra lại trước khi dựng và POST FHIR Bundle để tránh phát tán dữ liệu bệnh án qua endpoint cấu hình sai.
