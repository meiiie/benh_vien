...
- `scripts/harness/web-app-composition.mjs`: kiểm tra `App.tsx` vẫn là composition root nhỏ, frontend không gọi `fetch` trực tiếp ngoài API client, route nghiệp vụ đi qua feature adapter, landing không hard-code số liệu dễ trôi, FHIR document Bundle preview luôn gửi `x-consent-reference`/`x-recipient-organization-id`, và API client vẫn đọc được lỗi FHIR `OperationOutcome` từ `application/fhir+json`.
