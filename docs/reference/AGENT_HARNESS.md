...
- `scripts/harness/release-workflow.mjs`: kiểm tra workflow release chỉ publish GHCR image theo semantic version, không publish tag `latest`, giữ quyền workflow tối thiểu, đồng thời bật provenance/SBOM attestation và OCI labels truy vết cho image API và web.
