...
- **Gói chuyển hồ sơ là workflow, không phải bản sao dữ liệu lâm sàng.** `RecordTransfer` giữ trạng thái gửi/nhận/lỗi/thử lại, cơ sở gửi/nhận, consent, lý do chuyển và `bundleId`; đơn vị nhận phải có endpoint FHIR REST hỗ trợ `Bundle` trong Provider Directory trước khi tạo/gửi gói. Retry worker chỉ đưa gói lỗi đã đến hạn về hàng đợi `ready` và ghi audit, không giả lập bên nhận đã nhận hồ sơ. Khi xuất chuẩn, `RecordTransfer` thành FHIR `Task` trỏ tới `Bundle`, không nhân đôi toàn bộ hồ sơ bệnh án.
