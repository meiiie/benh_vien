...
- `RecordTransfer` là lớp vận hành nội bộ để chuẩn bị cho MHD/XDS trong tương lai; nó không thay thế `DocumentReference`, `Composition` hoặc `Bundle`, mà theo dõi việc gửi gói hồ sơ tới bên nhận. Trước khi tạo/gửi, đơn vị nhận phải có endpoint FHIR REST hỗ trợ `Bundle`; sau đó hệ thống ghi một delivery attempt dạng outbox cho từng lần gửi, kèm idempotency key để chống gửi trùng. Delivery worker có thể POST FHIR Bundle sang endpoint đích và ghi `succeeded/failed`; retry worker chỉ đưa gói lỗi đã đến hạn về hàng đợi gửi lại khi còn lượt thử, hoặc chuyển sang `dead-lettered` để nhân sự vận hành xử lý khi quá trần retry. Khi bên nhận xác nhận thủ công hoặc gửi callback acknowledgement hợp lệ, hệ thống lưu người xác nhận và mã biên nhận kỹ thuật; đây là biên nhận vận hành, chưa thay thế chữ ký số, giao dịch IHE MHD hoặc gateway xác thực hai chiều. Đây vẫn chưa phải IHE MHD đầy đủ vì chưa có profile/transaction MHD và registry tài liệu chuẩn hóa.
