...
- Kiểm tra biên kiến trúc API: route chuyển hồ sơ, route bệnh nhân, route ClinicalDocument, route MedicationAdministration và route Procedure không phình lại thành God route; các nhánh registry/query/command/FHIR resource/FHIR Bundle/callback phải nằm trong module route chuyên biệt, và helper command/audit/reference validation không được gom ngược vào route chính.
