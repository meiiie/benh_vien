...
- Kiểm tra biên FHIR type: `fhir-types.ts`, `fhir-clinical.types.ts`, `fhir-medication.types.ts`, `fhir-careflow.types.ts` và `fhir-diagnostics.types.ts` chỉ là barrel tương thích; các resource type phải nằm trong module `fhir-*.types.ts` theo nhóm để không quay lại God file.
