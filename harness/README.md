...
- Kiểm tra biên kiến trúc domain: `RecordTransfer`, `RecordTransferDeliveryAttempt`, `ProviderDirectory`, `AuditEvent`, `AccessControl`, `Patient`, `WorkflowTask`, `Procedure`, `ServiceRequest`, `ImagingStudy`, `ClinicalDocument` và miền thuốc giữ behavior tách khỏi type, snapshot, command input/code set, role-purpose-permission catalog.
