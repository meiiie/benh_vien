...
- Kiểm tra biên kiến trúc API: HTTP API composition root, route chuyển hồ sơ, route bệnh nhân, route ClinicalDocument, route MedicationDispense, route MedicationAdministration và route Procedure không phình lại thành God route; các nhánh domain wiring, registry/query/command/FHIR resource/FHIR Bundle/callback phải nằm trong module chuyên biệt, và helper command/audit/reference validation/transfer context/consent guard không được gom ngược vào route chính.
