...
- Kiểm tra biên kiến trúc API: HTTP API composition root, route xác thực, route chuyển hồ sơ, route Encounter, route bệnh nhân, route ClinicalDocument, route MedicationRequest, route MedicationDispense, route MedicationAdministration, route Procedure, route DiagnosticReport và route ImagingStudy không phình lại thành God route; các nhánh domain wiring, login/session/audit, registry/query/command/FHIR resource/FHIR Bundle/callback/send-receive-failure command phải nằm trong module chuyên biệt, và helper command/audit/reference validation/transfer context/consent guard không được gom ngược vào route chính.
