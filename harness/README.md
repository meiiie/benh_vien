...
Harness này khóa boundary persistence cho luồng chuyển hồ sơ bệnh án, hồ sơ bệnh nhân, WorkflowTask, Procedure và AuditEvent: repository chỉ điều phối truy vấn và transaction, SQL nằm trong module SQL, mapper chỉ chuyển row/tham số, còn command persistence chỉ ghép queryable với SQL/mapper. Với AuditEvent, repository vẫn được giữ vai trò điều phối domain sealing và integrity chain, nhưng không sở hữu SQL dài hoặc JSON mapping. Mục tiêu là giữ lớp hạ tầng có thể mở rộng theo DDD mà không để logic domain, SQL dài và transaction bị trộn lẫn trong một file lớn.
