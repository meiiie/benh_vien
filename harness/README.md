...
- Kiểm tra biên test API: test login/token, runtime/readiness/security envelope, startup/production config, patient registry/merge, ABAC theo phạm vi bệnh nhân và audit/AuditEvent nằm trong file riêng, test helper auth dùng chung không phình thành nơi chứa scenario, và suite auth/RBAC chính không nuốt ngược các nhóm này.
