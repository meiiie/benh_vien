...
- Kiểm tra biên FHIR type: `fhir-types.ts`, `fhir-clinical.types.ts` và `fhir-medication.types.ts` chỉ là barrel tương thích; các resource type phải nằm trong module `fhir-*.types.ts` theo nhóm để không quay lại God file.
