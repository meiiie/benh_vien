...
Harness này khóa boundary persistence cho luồng chuyển hồ sơ bệnh án, hồ sơ bệnh nhân, WorkflowTask và Procedure: repository chỉ điều phối truy vấn và transaction, SQL nằm trong module SQL, mapper chỉ chuyển row/tham số, còn command persistence chỉ ghép queryable với SQL/mapper. Mục tiêu là giữ lớp hạ tầng có thể mở rộng theo DDD mà không để logic domain, SQL dài và transaction bị trộn lẫn trong một file lớn.
