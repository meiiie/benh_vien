...
- Kiểm tra biên kiến trúc API: HTTP API composition root, route xác thực, route Consent, route chuyển hồ sơ, route Encounter, route Observation, route Condition, route AllergyIntolerance, route bệnh nhân, route ClinicalDocument, route MedicationRequest, route MedicationDispense, route MedicationAdministration, route ServiceRequest, route WorkflowTask, route Procedure, route DiagnosticReport và route ImagingStudy không phình lại thành God route; các nhánh domain wiring, login/session/audit, consent query/create/revoke/FHIR, registry/query/command/FHIR resource/FHIR Bundle/callback/send-receive-failure command phải nằm trong module chuyên biệt, và helper command/audit/reference validation/transfer context/consent guard không được gom ngược vào route chính.
