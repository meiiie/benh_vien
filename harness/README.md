...
- Kiểm tra biên FHIR type: `fhir-types.ts`, `fhir-clinical.types.ts`, `fhir-clinical-core.types.ts`, `fhir-medication.types.ts`, `fhir-careflow.types.ts` và `fhir-diagnostics.types.ts` chỉ là barrel tương thích; các resource type phải nằm trong module `fhir-*.types.ts` theo nhóm để không quay lại God file.
- Kiểm tra biên kiến trúc API: route chuyển hồ sơ và route bệnh nhân không phình lại thành God route; các nhánh registry/query/command/FHIR resource/FHIR Bundle/callback phải nằm trong module route chuyên biệt, và helper command/audit không được gom ngược vào route chính.
...
- Kiểm tra biên kiến trúc frontend: `App.tsx` không phình to, HTTP đi qua API client, formatter không bị gom lẫn miền, type lâm sàng phải import từ module nghiệp vụ chuyên biệt thay vì barrel tương thích `types/clinical.ts`, UI chuyển hồ sơ không gom ngược delivery-attempt/tóm tắt vận hành vào panel chính, renderer hồ sơ lâm sàng tách hợp đồng type khỏi phần lắp JSX, và panel tài liệu bệnh án/dị ứng an toàn/chẩn đoán/y lệnh dịch vụ/thuốc/care workflow/LIS/RIS/PACS không gom form command nhiều trường vào panel hiển thị.
