...
- Với FHIR type, `fhir-types.ts`, `fhir-clinical.types.ts` và `fhir-medication.types.ts` chỉ là barrel tương thích; resource declaration nằm trong các module `fhir-*.types.ts` theo nhóm shared/provider/document/privacy/audit/clinical/medication request-dispense-administration/careflow/diagnostics/patient/operation/capability/bundle.
