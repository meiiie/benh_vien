...
- Với FHIR type, `fhir-types.ts`, `fhir-clinical.types.ts`, `fhir-medication.types.ts`, `fhir-careflow.types.ts` và `fhir-diagnostics.types.ts` chỉ là barrel tương thích; resource declaration nằm trong các module `fhir-*.types.ts` theo nhóm shared/provider/document/privacy/audit/clinical/medication request-dispense-administration/careflow service request-task-procedure/diagnostics report-imaging/patient/operation/capability/bundle.
