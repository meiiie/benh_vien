...
- Type của bệnh nhân, lượt khám, dị ứng, chẩn đoán/vấn đề sức khỏe, quan sát/kết quả đo, thuốc, tài liệu lâm sàng, danh bạ nhà cung cấp, chuyển hồ sơ, đồng ý chia sẻ và audit nằm trong `src/types/patientRegistry.ts`, `src/types/encounters.ts`, `src/types/allergies.ts`, `src/types/conditions.ts`, `src/types/observations.ts`, `src/types/medications.ts`, `src/types/clinicalDocuments.ts`, `src/types/providerDirectory.ts`, `src/types/recordTransfers.ts`, `src/types/consents.ts` và `src/types/audit.ts`; `src/types/clinical.ts` chỉ re-export lại trong giai đoạn tách dần để giữ tương thích import cũ.
