...
- Type runtime/auth/app shell nằm trong `src/types/appRuntime.ts`. Type của bệnh nhân, lượt khám, dị ứng, chẩn đoán/vấn đề sức khỏe, quan sát/kết quả đo, thuốc, y lệnh/tác vụ/thủ thuật, xét nghiệm/chẩn đoán hình ảnh, tài liệu lâm sàng, danh bạ nhà cung cấp, chuyển hồ sơ, đồng ý chia sẻ và audit nằm trong module type tương ứng dưới `src/types`; `src/types/clinical.ts` chỉ là compatibility barrel để giữ tương thích import cũ.
