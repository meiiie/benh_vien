...
- Type của bệnh nhân, danh bạ nhà cung cấp và chuyển hồ sơ nằm trong `src/types/patientRegistry.ts`, `src/types/providerDirectory.ts` và `src/types/recordTransfers.ts`; `src/types/clinical.ts` chỉ re-export lại trong giai đoạn tách dần để giữ tương thích import cũ.
