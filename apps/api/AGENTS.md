...
- Test helper dùng chung cho auth/RBAC boundary nên nằm trong `server.auth.test-support.ts`; test login/token nằm trong `server.auth.login.test.ts`; test readiness/runtime/security header/error envelope nằm trong `server.runtime.test.ts`; file `server.auth.test.ts` ưu tiên giữ scenario tích hợp RBAC, ABAC, FHIR, audit và assertion. Chạy `pnpm run harness:api-test-composition` khi đổi các file test auth boundary.
