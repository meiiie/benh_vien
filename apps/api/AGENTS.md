...
- Route bệnh nhân phải giữ boundary theo module: `patient-routes.ts` chỉ làm composition root; danh sách/tạo hồ sơ nằm trong `patient-registry-routes.ts`; merge/đối soát hồ sơ nằm trong `patient-merge-routes.ts`; đọc hồ sơ nằm trong `patient-query-routes.ts`; xuất FHIR nằm trong `patient-fhir-routes.ts`. Chạy `pnpm run harness:api-route-composition` khi đổi các file này.
