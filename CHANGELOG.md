...
- Mở rộng rào ABAC theo bệnh nhân sang `Encounter`, `AllergyIntolerance`, `Condition`, `ClinicalDocument`, `Observation`, `ServiceRequest`, `Task` và `RecordTransfer` để chặn truy cập qua endpoint con khi người dùng điều trị không thuộc tổ chức quản lý hồ sơ.
