...
- Chặn các request ghi dữ liệu vào hồ sơ bệnh nhân đã merge: các route dùng `requirePatientRecordAccessByPatientId` trả `409 PATIENT_RECORD_MERGED` cho thao tác tạo/cập nhật sau merge, đồng thời giữ quyền đọc/xuất FHIR để truy vết lịch sử.
- Đóng cổng API trong prod-like Docker compose: chỉ web edge được publish ra host, CI/readiness kiểm API từ bên trong container và harness chặn việc publish trực tiếp API port.
- Gia cố web edge Nginx bằng `server_tokens off`, `Cross-Origin-Opener-Policy`, `Strict-Transport-Security` và `X-Request-Id` proxy header để giảm lộ thông tin và giữ trace correlation.
- Pin image runtime của MinIO, HAPI FHIR và Orthanc trong Docker compose, đồng thời thêm harness chặn image dạng `latest` hoặc thiếu tag để tránh trôi phiên bản ngoài kiểm soát.
- Chặn khởi động production nếu thiếu secret callback nhận hồ sơ hoặc secret callback vẫn là placeholder/dev-only, thay vì chỉ báo lỗi khi callback được gọi.
- Bổ sung luồng merge hồ sơ bệnh nhân ở lớp Patient Registry/MPI: admin có thể đánh dấu hồ sơ trùng đã nhập vào hồ sơ đích, lưu `merged_into_patient_id`, thời điểm, người thực hiện, lý do, audit `patient.merge` và FHIR `Patient.link` kiểu `replaced-by`.
- Bổ sung rào định danh bệnh nhân/MPI tối thiểu: domain chặn trùng định danh trong cùng hồ sơ, API trả `409 PATIENT_IDENTIFIER_CONFLICT` khi CCCD/BHYT/MRN đã thuộc hồ sơ khác, PostgreSQL có bảng `patient_identifier_index` với unique constraint và smoke test xác nhận không tạo hồ sơ trùng định danh.
- Bổ sung audit event `access.denied` cho phản hồi `403 FORBIDDEN` và `403 PATIENT_ACCESS_DENIED`, giúp kiểm toán viên thấy được các lần cố truy cập hồ sơ bị chặn và xuất được dưới dạng FHIR `AuditEvent` có outcome thất bại.
- Bổ sung `GET /api/v1/audit-events` để kiểm toán viên hoặc quản trị viên có mục đích `AUDIT` xem các audit event gần nhất ở phạm vi toàn hệ thống, gồm cả sự kiện bảo mật không gắn trực tiếp với một bệnh nhân.
- Bổ sung audit event `auth.login.success` và `auth.login.failure` cho boundary đăng nhập demo; sự kiện thất bại chỉ lưu `usernameHash`, không lưu username thô.
- Bổ sung ABAC tối thiểu cho Patient Registry: người dùng điều trị chỉ thấy/truy cập bệnh nhân thuộc tổ chức điều trị trong Provider Directory, có tính quan hệ bệnh viện-khoa/phòng.
- Mở rộng rào ABAC theo bệnh nhân sang `Encounter`, `AllergyIntolerance`, `Condition`, `MedicationRequest`, `MedicationDispense`, `MedicationAdministration`, `ClinicalDocument`, `Observation`, `ServiceRequest`, `Task`, `Procedure`, `DiagnosticReport`, `ImagingStudy`, `Consent`, `AuditEvent` và `RecordTransfer` để chặn truy cập qua endpoint con khi người dùng điều trị không thuộc tổ chức quản lý hồ sơ.
...
- Chặn `BVS_RATE_LIMIT_STORE=memory` trong production để tránh rate limit đăng nhập chỉ hoạt động cục bộ từng replica.
- Bổ sung readiness check cho kho rate limit đăng nhập để `/ready` trả `503 not_ready` khi Valkey không sẵn sàng.
- Bổ sung guard cấu hình repository: chỉ nhận `postgres` hoặc `in-memory`, riêng production bắt buộc `BVS_REPOSITORY=postgres`.
- Bổ sung validation `BVS_PUBLIC_API_BASE_URL` để production FHIR `CapabilityStatement` chỉ công bố URL API HTTPS public và chặn `localhost`, loopback, private IP hoặc link-local IP.
...
- Bổ sung `BVS_DEMO_AUTH_ENABLED` để tắt đăng nhập demo mặc định trong production, chỉ bật khi smoke/demo có kiểm soát.
...
- Siết validation `BVS_CORS_ORIGINS` ở production, chỉ chấp nhận Origin HTTPS canonical public và chặn wildcard, HTTP, path, localhost, loopback, private IP hoặc link-local IP.
