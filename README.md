...
API có `/health` cho liveness, `/ready` cho readiness và `/api/v1/runtime` cho metadata vận hành an toàn như phiên bản API, repository, public base URL, trạng thái bật/tắt tài liệu API và các worker chuyển hồ sơ. `/ready` kiểm tra API đọc được repository bệnh nhân và Provider Directory trước khi container được xem là sẵn sàng nhận traffic.
