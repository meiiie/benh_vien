...
- `Interop`: kiểm tra FHIR `Patient`, `Encounter`, `AllergyIntolerance`, `Condition`, `ServiceRequest`, `Observation`, `MedicationRequest`, `DocumentReference`, `Bundle` và các hướng mở sang HIS/LIS/PACS.
...
Backend hiện dùng Fastify + TypeScript. Trong Docker, API chạy với `BVS_REPOSITORY=postgres`, migration service tạo schema PostgreSQL trước khi API khởi động. Các bảng nền tảng gồm `patients`, `encounters`, `allergy_intolerances`, `conditions`, `service_requests`, `observations`, `medication_requests`, `clinical_documents`, `consents`, `audit_events` và `schema_migrations`.
...
- Xuất gói hồ sơ bệnh nhân sang FHIR `Bundle` dạng `collection` gồm Patient, Encounter, AllergyIntolerance, Condition, ServiceRequest, Observation, MedicationRequest và DocumentReference; API chỉ cho xuất khi consent còn hiệu lực và khớp đơn vị nhận.
